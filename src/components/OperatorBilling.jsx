...
const PlanCard = ({ id, name, price, features, current, selected, starter, onSelect }) => (
  <button
    type="button"
    className={`operator-plan-card${starter ? " operator-plan-card--starter" : ""}`}
    onClick={() => onSelect(id)}
    style={{ ...styles.plan, ...(starter ? styles.starter : {}), ...((current || selected) ? styles.selected : {}) }}>
    {
      (current || starter) && <span className={`operator-plan-badge${current ? " operator-plan-badge--current" : ""}`} style={current ? styles.currentBadge : styles.recommendedBadge}>{
        current ? "CURRENT PLAN" : "RECOMMENDED"}
      </span>
    }
    <span
      className="operator-plan-title"
      style={{ ...styles.planTitle, color: starter ? "#10b981" : "#fff" }}>
      {
        starter ? <img className="starter-plan-icon" src={lightningIcon} alt="" style={styles.planIcon} /> : <Store size={21} />
      }
      <strong>{name}</strong>
    </span>
    <span style={styles.price}><span className={`operator-plan-currency${starter ? " starter-plan-currency" : ""}`}
      style={{ ...styles.currency, color: starter ? "#10b981" : "inherit" }}>$</span><strong className="price-amount" style={{ ...styles.priceAmount, color: starter ? "#10b981" : "inherit" }}>{price}</strong><small>/month</small></span>
    {starter && <span style={styles.spotlight}>★ Includes Spotlight — be seen first</span>}
    <span style={styles.features}>{features.map((feature) => <span key={feature}
      style={styles.feature}>
      <CheckCircle2 className="operator-plan-feature-check" size={17} fill={starter ? "#10b981" : "#94a3b8"}
        color={starter ? "#063b2c" : "#334155"} /><span>{feature}</span></span>)}
    </span>
...
const styles = {
  header: { marginBottom: "28px" }, heading: { margin: "0 0 7px", color: "#10b981", fontSize: "32px" },
  subheading: { margin: 0, color: "#64748b", fontSize: "15px" },
  ...
  plan: {
    position: "relative", display: "flex", flexDirection: "column", minHeight: "520px",
    padding: "30px", borderRadius: "20px", border: "1px solid rgba(255,255,255,.15)",
    background: "#0d2927", color: "rgba(255,255,255,.75)", font: "inherit", textAlign: "left",
    cursor: "pointer", overflow: "visible"
  },
  starter: { background: "#062d25", borderColor: "rgba(16,185,129,.55)" },
  selected: {
    borderColor: "#10b981",
    boxShadow: "0 0 0 1px #10b981, 0 18px 40px rgba(0,0,0,.18)"
  },
  currentBadge: {
    position: "absolute", top: "-11px", right: "22px", padding: "5px 12px",
    borderRadius: "999px", background: "#16a34a", color: "#031d16",
    fontSize: "10px", fontWeight: 800
  },
  recommendedBadge: {
    position: "absolute", top: "-11px", right: "22px",
    padding: "5px 12px", borderRadius: "999px",
    background: "#34d399", color: "#031d16", fontSize: "10px", fontWeight: 800
  },
  planTitle: {
    display: "flex",
  ...
  planIcon: { width: "22px", height: "22px", flexShrink: 0, objectFit: "contain" },
  price: {
  ...
  feature: {
    ...
    fontSize: "14px", lineHeight: 1.35, overflowWrap: "anywhere"
  },
  spotlight: {
    margin: "0 0 22px", padding: "9px 12px", borderRadius: "8px",
    background: "rgba(16,185,129,.14)", color: "#34d399", fontSize: "13px", fontWeight: 700
  },
  selection: {
    display: "flex", justifyContent: "center", alignItems: "center", gap: "9px",
    marginTop: "auto", paddingTop: "19px", borderTop: "1px solid rgba(255,255,255,.1)",
  ...
  radioInner: { width: "8px", height: "8px", borderRadius: "50%", background: "#10b981" },
  action: {
    width: "100%", marginTop: "28px", padding: "16px", border: 0, borderRadius: "14px",
    background: "linear-gradient(90deg,#40ea54,#04ca8f)",
    color: "#03231d", fontSize: "16px", fontWeight: 800, cursor: "pointer",
